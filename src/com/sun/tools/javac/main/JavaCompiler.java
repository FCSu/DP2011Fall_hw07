...
//      System.err.println("completing " + c);//DEBUG
     ...
     */
    public void compile(List<JavaFileObject> sourceFileObjects,  //\\ 程式主軸
	...
	//\\ 軟體設計模式作業 增添始
        try {
            ...
            Class clazz = Class.forName("hw07." + groupID + ".NodeCountingVisitor");
            java.lang.reflect.Constructor init = clazz.getConstructor();
            java.lang.reflect.Method list = clazz.getMethod("list");
            
            JCTree.Visitor visitor = (JCTree.Visitor)init.newInstance();
                                     // 相當於 new hw07.group88.NodeCountingVisitor();
            java.util.Iterator<Env<AttrContext>> iterator = todo.iterator();
            while (iterator.hasNext()) {
                System.out.println("has one"); 
                JCCompilationUnit jccu = iterator.next().toplevel;
                // 注意：某些 JCCompilationUnit 可能會被重複 iteration 到，但只能計算一個
                //       其他的 Node 也有可能會被重複 traversal 到
                //       即所謂的 parsing tree 不是純粹的 tree，有可能是 graph
                //       重複 traversal 到的 node，也只能計算一個
                jccu.accept(visitor);  // 進行統計
            }
            String[] result = (String[])list.invoke(visitor);  // 印出結果
                              // 相當於 visitor.list();
            
            for (String s: result)
                System.out.println(s);
            // 把結果 serialized 到檔案，以便進行 JUinit
            FileOutputStream fos = new FileOutputStream("test" + File.separatorChar + "result_" + groupID);
